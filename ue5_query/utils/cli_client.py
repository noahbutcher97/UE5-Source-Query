...
def main():
    ...
    parser.add_argument("--describe", action="store_true",
                       help="Output machine-readable tool schema (JSON)")
    ...
    if not args.question:
        print("[ERROR] Question required (unless using --batch-file or --describe)", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
    ...
    try:
        ...
        if not results:
            ...
            try:
                ...
                if not (data_dir / "vector_store.npz").exists():
                    error_msg = f"Search index not found at: {data_dir}. Run 'launcher.bat' or 'tools/rebuild-index.bat'"
                    if args.format in ('json', 'jsonl') or args.json:
                ...
                if args.filter:
                    ...
                    try:
                        ...
                        if not args.json and args.format == "text":
        ...
        # Check if JSON output is expected (Legacy flag OR format=json)
        wants_json = args.json or args.format in ('json', 'jsonl')
